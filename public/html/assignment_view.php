...
<link rel="stylesheet" href="../css/assignment_view_styles.css"> <div class="course-view-main-content"> <?php if ($assignment_data): ?>
    ...
    <?php else: ?>
        <div class="course-not-found"> <h1>Помилка</h1>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadAssignmentAndSubmissionDetails(assId) {
        ...
        // Оновлення UI елементів, які можуть бути заповнені одразу з PHP (title, description etc.)
        // тут не потрібне, оскільки PHP вже їх вивів.
        // Ця функція буде в основному для завантаження статусу здачі студента або форми.

        if (!isTeacher && studentSubmissionArea) { // Тільки для студента
            ...
            try {
                ...
                if (result.status === 'success') {
                    // result.assignment_details містить дані завдання, але вони вже є в PHP
                    // result.submission_details містить дані здачі студента
                    // result.is_teacher_of_course - можна перевірити ще раз, якщо потрібно
                    ...
                    if (result.submission_details) {
                        ...
                        if (submission.file_path) {
                            ...
                            // Шлях до файлу відносно поточної сторінки (public/html/...)
                        ...
                        let statusClass = '';
                        // Визначення тексту та класу для статусу
    ...
    function showSubmissionForm() {
        ...
        const assignmentMaxPoints = <?php echo json_encode($assignment_data['max_points'] ?? 'N/A'); ?>; // Отримуємо з PHP
    ...
    function validateSubmissionFile(event) {
        ...
        if (file) {
            ...
            // Перевірка розширень (можна додати mime types для надійності)
    ...
    function nl2br(str) { // Проста функція nl2br для JS
    ...
    if (assignmentId) { // Завантажуємо деталі, якщо є ID завдання
